...
from .v2 import GuardV2Pipeline, discover_video_assets, load_guard_v2_config, write_outputs
...
def main() -> None:
    ...
    logger.info("Guard V2 analysis complete")
    logger.info("Total videos: %s", summary["total_videos"])
    logger.info("Completed videos: %s", summary["completed_videos"])
    logger.info("Failed videos: %s", summary["failed_videos"])
    logger.info("Safe to watch: %s", summary["safe_to_watch_count"])
    logger.info("Average safety score: %s", summary["average_safety_score"])
    logger.info("Average risk score: %s", summary["average_risk_score"])
    logger.info("Average network diffusion risk: %s", summary["average_network_diffusion_risk"])
    logger.info("Average analysis confidence: %s", summary["average_analysis_confidence"])
    logger.info("High risk count: %s", quality_summary["high_risk_count"])
