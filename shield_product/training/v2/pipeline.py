...
class GuardV2Pipeline:
    ...
    def analyze_asset(self, asset: MediaAsset) -> Dict[str, object]:
        signals = self.signal_extractor.extract(
            ...
            keyframe_interval_seconds=float(self.config.keyframe_interval_seconds),
            whisper_model=str(self.config.whisper_model_name),
        ...
        modality_scores = self._build_modality_scores(signals=signals, harm_score=harm_score)

        learned_fusion_score = None
        if bool(self.config.use_learned_fusion):
            learned_fusion_score = compute_learned_modality_fusion(
                modality_scores=modality_scores,
                model_path=str(self.config.fusion_model_path),
            )

        if learned_fusion_score is not None:
            fusion_score = float(learned_fusion_score)
        ...
        final_risk = compute_final_risk(
            ...
            modality_fusion_weight=float(self.config.fusion_blend_weight),
        )
        ...
        verdict = verdict_from_risk(
            ...
            safe_threshold=float(self.config.safe_threshold),
            review_threshold=float(self.config.review_threshold),
            block_threshold=float(self.config.block_threshold),
        )
        ...
        return {
            ...
            "fusion": {
                ...
                "blend_weight": round(float(self.config.fusion_blend_weight), 4),
            },
        }
    ...
    def _build_modality_scores(
        ...
        *,
        ...
        text_score = max(0.0, min(float(harm_score), 100.0))
        ...
        image_score = max(
            ...
            min(
                float(
                    max(
                        signals.frame_signals.get("clip_hate_score", 0.0),
                        signals.frame_signals.get("clip_explicit_score", 0.0),
                        signals.frame_signals.get("clip_max_risk", 0.0),
                    )
                ),
                100.0,
            ),
        ...
        if signals.transcript_text.strip():
            audio_score = text_score
        else:
            audio_score = max(
                0.0,
                min(float(signals.metadata.get("audio_transcript_risk", 0.0) or 0.0), 100.0),
            )

        video_score = max(text_score, image_score)
