...
def compute_viral_harm_potential(
    ...
    return round(clamp(potential), 4)
...
def compute_network_diffusion_risk(
    ...
    content_activation = min(
        max(
            (
                0.55 * float(safety_harm_score)
                + 0.25 * float(contradiction_score)
                + 0.20 * float(evasion_score)
            )
            / 100.0,
            0.0,
        ),
...
def compute_final_risk(
    ...
    modality_fusion_score: Optional[float] = None,
    modality_fusion_weight: float = 0.35,
) -> float:
    baseline = (
    ...
    if modality_fusion_score is None:
        return round(clamp(baseline), 4)

    blend = max(0.0, min(float(modality_fusion_weight), 1.0))
    fused_value = (1.0 - blend) * baseline + blend * float(modality_fusion_score)
    return round(clamp(fused_value), 4)


def compute_weighted_modality_fusion(
    modality_scores: Mapping[str, float],
    weights: Optional[Mapping[str, float]] = None,
) -> float:
    weight_map = _normalize_weight_map(weights or DEFAULT_MODALITY_WEIGHTS)
    weighted_sum = 0.0
    for modality in ("text", "image", "audio", "video"):
        score = clamp(safe_float(modality_scores.get(modality, 0.0)))
        weighted_sum += score * float(weight_map.get(modality, 0.0))
    return round(clamp(weighted_sum), 4)


def compute_learned_modality_fusion(
    modality_scores: Mapping[str, float],
    model_path: str,
) -> Optional[float]:
    ...
    if joblib is None or np_module is None:
        logger.warning(
            "learned fusion model could not run because joblib/numpy is unavailable"
        )
        return None
    ...
    try:
        ...
        logger.warning("failed to load fusion model at %s: %s", resolved_path, exc)
        return None

    feature_vector = np_module.asarray(
        [
            [
                clamp(safe_float(modality_scores.get("text", 0.0))),
                clamp(safe_float(modality_scores.get("image", 0.0))),
                clamp(safe_float(modality_scores.get("audio", 0.0))),
                clamp(safe_float(modality_scores.get("video", 0.0))),
            ]
        ],
        dtype=float,
    )
    ...
    try:
        if hasattr(model, "predict_proba"):
            ...
            return round(clamp(probability * 100.0), 4)

        if hasattr(model, "decision_function"):
            logit = float(model.decision_function(feature_vector)[0])
            probability = 1.0 / (1.0 + math.exp(-logit))
            return round(clamp(probability * 100.0), 4)

        if hasattr(model, "predict"):
            prediction = safe_float(model.predict(feature_vector)[0])
            if 0.0 <= prediction <= 1.0:
                return round(clamp(prediction * 100.0), 4)
            return round(clamp(prediction), 4)
    except (AttributeError, RuntimeError, TypeError, ValueError) as exc:
        logger.warning("learned fusion inference failed: %s", exc)
        return None

    logger.warning("learned fusion model at %s is unsupported", resolved_path)
    return None
...
def verdict_from_risk(
    ...
    risk_score = max(float(final_risk), float(network_diffusion))
    if risk_score >= float(block_threshold):
        return "BLOCK"
    if risk_score >= float(review_threshold):
        return "REVIEW"
    if risk_score < float(safe_threshold):
        return "SAFE"
    return "REVIEW"
...
def _normalize_weight_map(weights: Mapping[str, float]) -> Dict[str, float]:
    ...
    for modality in ("text", "image", "audio", "video"):
        value = max(0.0, safe_float(getattr(weights, modality, 0.0)))
    ...
    for modality in normalized:
        normalized[modality] = normalized[modality] / total
    return normalized
